...
void emu_drawText(unsigned short x, unsigned short y, const char * text, unsigned short fgcolor, unsigned short bgcolor, int doublesize)
{
  ...
  draw_text(text, x, y, fgcolor, bgcolor);
}
...
static inline void adc_select_input(unsigned input) {  }
...
static int readNbFiles(char * rootdir) {
  ...
  DIR dir;
  FILINFO entry;
  FRESULT fr = f_opendir(&dir, rootdir) | f_readdir(&dir, &entry);
...
void backgroundMenu(void) {
    ...
    draw_text(TITLE, 0, 0, RGBVAL16(0x00,0xff,0xff), RGBVAL16(0x00,0x00,0xff));
}
...
///    tft.drawRectNoDma(MENU_FILE_XOFFSET,MENU_FILE_YOFFSET, MENU_FILE_W, MENU_FILE_H, MENU_FILE_BGCOLOR);
      ...
      if (fileIndex >= topFile) {              
        if ((i+topFile) < nbFiles ) {
          if ((i+topFile)==curFile) {
            ...
            draw_text(filename, MENU_FILE_XOFFSET, i * TEXT_HEIGHT + MENU_FILE_YOFFSET, RGBVAL16(0xff,0xff,0x00), RGBVAL16(0xff,0x00,0x00));
            ...
            draw_text(filename, MENU_FILE_XOFFSET, i * TEXT_HEIGHT + MENU_FILE_YOFFSET, RGBVAL16(0xff,0xff,0xff), MENU_FILE_BGCOLOR);
          }
        }
        i++; 
      }
    ...
    draw_text((emu_SwapJoysticks(1)?(char*)"SWAP=1":(char*)"SWAP=0"), 48, MENU_JOYS_YOFFSET + 8, RGBVAL16(0x00,0xff,0xff), RGBVAL16(0x00,0x00,0xff));
    menuRedraw=false;     
  }
...
static bool emu_eraseConfig(void)
{
  return FR_OK == f_unlink (ROMSDIR "/" AUTORUN_FILENAME);
}
