...
void cia1_write(uint32_t address, uint8_t value) {
	address &= 0x0F;
	if ( address == 0x04 ) { cpu.cia1.W[address] = value; } //Timer A LO
	else if ( address == 0x05 ) { cpu.cia1.W[address] = value; if ((cpu.cia1.R[0x0E] & 0x01) == 0) cpu.cia1.R[address]=value; } //Timer A HI
	else if ( address == 0x06 ) { cpu.cia1.W[address] = value; } //Timer B LO
	else if ( address == 0x07 ) { cpu.cia1.W[address] = value; if ((cpu.cia1.R[0x0F] & 0x01) == 0) cpu.cia1.R[address]=value; } //Timer B HI
		//RTC
	...
	else if ( address == 0x0F ) {cpu.cia1.R[address] = value & ~0x10; if ((value & 0x10)>0) { cpu.cia1.R16[0x06/2] = cpu.cia1.W16[0x06/2]; }}
	else {cpu.cia1.R[address] = value;/*if (address ==0) {Serial.print(value);Serial.print(" ");}*/ }
...
uint8_t cia1_read(uint32_t address) {
	uint8_t ret;
	address &= 0x0F;
	if ( address == 0x00 ) {ret = cia1PORTA();}
	else if ( address == 0x01 ) {ret = cia1PORTB();}
		//RTC
	else if ( address == 0x08 ) {
					ret = tod() % 1000 / 10;
					...
					#endif
					//Bit 0..3: Zehntelsekunden im BCD-Format ($0-$9) Bit 4..7: immer 0
	}
	else if ( address == 0x09 ) {
					...
					Serial.println(ret,HEX);
					#endif
	} //Bit 0..3: Einersekunden im BCD-Format ($0-$9) Bit 4..6: Zehnersekunden im BCD-Format ($0-$5) Bit 7: immer 0
	else if ( address == 0x0A ) {
					...
					Serial.println(ret,HEX);
					#endif
	//Bit 0..3: Einerminuten im BCD-Format( $0-$9) Bit 4..6: Zehnerminuten im BCD-Format ($0-$5) Bit 7: immer 0
	}
	else if ( address == 0x0B ) {
					//Bit 0..3: Einerstunden im BCD-Format ($0-$9) Bit 4: Zehnerstunden im BCD-Format ($0-$1) // Bit 7: Unterscheidung AM/PM, 0=AM, 1=PM
					//Lesen aus diesem Register friert alle TOD-Register ein (TOD lauft aber weiter), bis Register 8 (TOD 10THS) gelesen wird.
					cpu.cia1.TODfrozen = 0;
...
void cia1_clock(int clk) {
	...
	// TIMER B
	//TODO : Prufen ob das funktioniert
