...
int CartridgeClass::LoadCartridgeImage(FILE *file)
{
    ...
    f_read(file, signature, 16, &reading_bytes);
    signature[16] = 0;
    ...
    f_read(file, &header_lenght, sizeof(header_lenght), &reading_bytes);
    header_lenght = ConvertDWord(header_lenght);

    f_read(file, &version, sizeof(version), &reading_bytes);

    f_read(file, &cartridge_type, sizeof(cartridge_type), &reading_bytes);
    cartridge_type = static_cast<uint16_t>(cartridge_type << 8) | (cartridge_type >> 8);

    f_read(file, &exrom, sizeof(exrom), &reading_bytes);
    f_read(file, &game, sizeof(game), &reading_bytes);
    ...
    *this->game = cartridge_game;
    ...
    chip_count = 0;
L1:
    f_lseek(file, akt_pos);
    f_read(file, signature, 4, &reading_bytes);
    ...
    if ( 0 == strcmp("CHIP", signature) )
    {
        ...
        else if ( chip_address == 0xA000 ) {
            reading_bytes = fread(rom_bank2 + (bank_pos * 0x2000),1,0x2000,file);
        }
        else if ( chip_address == 0xE000 ) {
            reading_bytes = fread(rom_bank2 + (bank_pos * 0x2000),1,0x2000,file);
        }
        chip_count ++;
        goto L1;
...
int CartridgeClass::GetCartridgeInfo(FILE *file, CARTRIDGE_INFO_STRUCT* crt_info)
{
    ...
    snprintf(crt_info->Version,10,"%X.%2.2X",static_cast<uint8_t>(version), version>>8);
...
L1:
    ...
    if(0==strcmp("CHIP",signature))
    {
        ...
        uint16_t t = crt_info->ChipInfo[crt_info->ChipCount].LoadAdress;
        if ( t == 0x8000 ) {
...
void CartridgeClass::Reset()
{
    ...
    {
        if ( cartridge_type == 1 ) { // Action Replay 4/5/6
            ar_active = true;
            WriteIO1(0xDE00, 0);
        }
        else if ( cartridge_type == 3 ) { //Final Cartgide III
            WriteIO2(0xDFFF, 0x00);
        }
        else if ( cartridge_type == 9 ) { //Atomic Power
        }
        else if ( cartridge_type == 32 ) { //EasyFlash
            WriteIO1(0xDE00,0);
            WriteIO1(0xDE02,0);
        }
        else {
...
void CartridgeClass::WriteIO1(uint16_t adresse, uint8_t value)
{
    ...
    else if(cartridge_type == 5)
    {
    		// Ocean type 1
        value &= 0x3F;
        lo_rom = rom_bank1 + (value * 0x2000);
    }
    else if(cartridge_type == 7)
    {
    		// Fun Play
        if(adresse == 0xDE00)
        {
    ...
    else if(cartridge_type == 19)
    {
        // Magic Desk Cartridge 512k (Bis jetzt read-only)
...
uint8_t CartridgeClass::ReadIO1(uint16_t address)
{
    ...
    if(cartridge_type == 3)	// Final Cartridge III
        return rom_bank1[0x1E00 + (rom_lo_bank << 13) + (address & 0xFF)].get();
    if(cartridge_type == 4) {
        if(address == 0xDE00)
        {
                *game = true;
...
void CartridgeClass::WriteIO2(uint16_t address, uint8_t value)
{
    ...
    else if(cartridge_type == 3 ) {	// Final Cartridge III
...
///            if(led_00 != led_00_old) if(ChangeLED != nullptr)  ChangeLED(0,led_00);
    ...
    else if(cartridge_type == 8) {		// Super Games
        lo_rom = rom_bank1 + ((value&0x3) * 0x2000);
        hi_rom = rom_bank2 + ((value&0x3) * 0x2000);
        if(value & 0x04)
        {
...
uint8_t CartridgeClass::ReadIO2(uint16_t address)
{
    ...
    if(cartridge_type == 32)	// EasyFlash
