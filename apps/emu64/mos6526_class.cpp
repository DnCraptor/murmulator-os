...
inline void MOS6526::TimerCount()
{
	...
	/// TimerA Status ///
    if ( timer_a_status == WAIT_THEN_COUNT ) {
            timer_a_status = COUNT;
            goto TimerA_Idle;
	}
    else if ( timer_a_status == STOP )
            goto TimerA_Idle;
    else if ( timer_a_status == LOAD_THEN_STOP ) {
            timer_a_status = STOP;
            timer_a = timer_a_latch;
			goto TimerA_Idle;
	}
    else if ( timer_a_status == LOAD_THEN_COUNT ) {
            timer_a_status = COUNT;
            timer_a = timer_a_latch;
			goto TimerA_Idle;
	}
    else if ( timer_a_status == LOAD_THEN_WAIT_THEN_COUNT ) {
    ...
    else if ( timer_a_status == COUNT ) {
			goto TimerA_Count;
	}
    else if ( timer_a_status == COUNT_THEN_STOP ) {
            timer_a_status = STOP;
			goto TimerA_Count;
	}
...
TimerA_Idle:
    if (writing_new_cra)
	{
        if ( timer_a_status == STOP || timer_a_status == LOAD_THEN_STOP ) {
                if (new_cra & 1)
				{
        ...
        else if ( timer_a_status == LOAD_THEN_COUNT || timer_a_status == WAIT_THEN_COUNT ) {
                if (new_cra & 1)
				{
                    if (new_cra & 8)
					{
	...
	/// TimerB Status ///
    if ( timer_b_status == WAIT_THEN_COUNT ) {
            timer_b_status = COUNT;
            goto TimerB_Idle;
	}
    else if ( timer_b_status == STOP )
            goto TimerB_Idle;
    else if ( timer_b_status == LOAD_THEN_STOP ) {
            timer_b_status = STOP;
            timer_b = timer_b_latch;
			goto TimerB_Idle;
	}
    else if ( timer_b_status == LOAD_THEN_COUNT ) {
            timer_b_status = COUNT;
            timer_b = timer_b_latch;
			goto TimerB_Idle;					//?????? Es Stand TimerA_Idle
	}
    else if ( timer_b_status == LOAD_THEN_WAIT_THEN_COUNT ) {
    ...
    else if ( timer_b_status == COUNT )
			goto TimerB_Count;
    else if ( timer_b_status == COUNT_THEN_STOP ) {
            timer_b_status = STOP;
			goto TimerB_Count;
	}
