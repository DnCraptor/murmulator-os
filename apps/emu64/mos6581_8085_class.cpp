...
const fc_point f0_points_6581[31]={{0,220},{0,220},{128,230},{256,250},{384,300},{512,420},{640,780},{768,1600},{832,2300},{896,3200},{960,4300},{992,5000},{1008,5400},{1016,5700},{1023,6000},{1023,6000},{1024,4600},{1024,4600},{1032,4800},{1056,5300},{1088,6000},{1120,6600},{1152,7200},{1280,9500},{1408,12000},{1536,14500},{1664,16000},{1792,17100},{1920,17700},{2047,18000},{2047,18000}};
const fc_point f0_points_8580[19]={{0,0},{0,0},{128,800},{256,1600},{384,2500},{512,3300},{640,4100},{768,4800},{896,5600},{1024,6500},{1152,7500},{1280,8400},{1408,9200},{1536,9800},{1664,10500},{1792,11000},{1920,11700},{2047,12500},{2047,12500}};
...
{
    unsigned short t = adresse & 0x1F;
    if ( t == 25 ) { // AD Wandler 1
        if(SidNummer == 0)
            return PotX;
    }
    else if ( t == 26 ) { // AD Wandler 2
        if(SidNummer == 0)
            return PotY;
    }
    else if ( t == 27 )
...
void MOS6581_8085::WriteIO(unsigned short adresse,unsigned char wert)
{
    ...
    else if ( t == 5 ) { // Attack-Decay für Stimme 0
    ...
    else if ( t == 6 ) { // Sustain-Release für Stimme 0
    ...
    else if ( t == 12 ) { // Attack-Decay für Stimme 1
    ...
    else if ( t == 13 ) { // Sustain-Release für Stimme 1
    ...
    else if ( t == 19 ) { // Attack-Decay für Stimme 2
    ...
    else if ( t == 20 ) { // Sustain-Release für Stimme 2
    ...
    else if ( t == 21 ) { // FilterfrequenzLO
        FilterFrequenz=(FilterFrequenz & 0x7F8) | (wert & 0x007);
        SetW0();
    }
    else if ( t == 22 ) { // FilterfrequenzHI
        FilterFrequenz=((wert<<3) & 0x7F8) | (FilterFrequenz & 0x007);
        SetW0();
    }
    else if ( t == 23 ) { // Filterkontrol_1 und Resonanz
        FilterResonanz=(wert>>4)&0x0F;
        SetQ();
        FilterKey=wert&0x0F;
    }
    else if ( t == 24 ) { // Filterkontrol_2 und Lautstärke
        Voice3Off=wert&0x80;
        HpBpLp=(wert>>4)&0x07;
        Volume=wert&0x0F;
    }
...
inline void MOS6581_8085::EnvZyklus(void)
{
    for(int i=0;i<3;i++)
    {
        ...
        v->RateCounter=0;
        if(t == ATTACK || ++v->ExponentialCounter == v->ExponentialCounterPeriod)
        {
...
inline void MOS6581_8085::FilterZyklus(int voice1,int voice2,int voice3,int ext_in)
{
    ...
    else if ( FilterKey == 0x02 ) {
        Vi = voice2;
        Vnf = voice1 + voice3 + ext_in;
    }
    else if ( FilterKey == 0x03 ) {
        Vi = voice1 + voice2;
        Vnf = voice3 + ext_in;
    }
    else if ( FilterKey == 0x04 ) {
        Vi = voice3;
        Vnf = voice1 + voice2 + ext_in;
    }
    else if ( FilterKey == 0x05 ) {
        Vi = voice1 + voice3;
        Vnf = voice2 + ext_in;
    }
    else if ( FilterKey == 0x06 ) {
        Vi = voice2 + voice3;
        Vnf = voice1 + ext_in;
    }
    else if ( FilterKey == 0x07 ) {
        Vi = voice1 + voice2 + voice3;
        Vnf = ext_in;
    }
    else if ( FilterKey == 0x08 ) {
        Vi = ext_in;
        Vnf = voice1 + voice2 + voice3;
    }
    else if ( FilterKey == 0x09 ) {
        Vi = voice1 + ext_in;
        Vnf = voice2 + voice3;
    }
    else if ( FilterKey == 0x0A ) {
        Vi = voice2 + ext_in;
        Vnf = voice1 + voice3;
    }
    else if ( FilterKey == 0x0B ) {
        Vi = voice1 + voice2 + ext_in;
        Vnf = voice3;
    }
    else if ( FilterKey == 0x0C ) {
        Vi = voice3 + ext_in;
        Vnf = voice1 + voice2;
    }
    else if ( FilterKey == 0x0D ) {
        Vi = voice1 + voice3 + ext_in;
        Vnf = voice2;
    }
    else if ( FilterKey == 0x0E ) {
        Vi = voice2 + voice3 + ext_in;
        Vnf = voice1;
    }
    else if ( FilterKey == 0x0F ) {
