...
REUClass::REUClass() : PsramBaenke(RamBaenke64k, 0x10000)
{
    REUInsert = false;
...
int REUClass::LoadRAM(const char *filename)
{
    ...
    for(int i = 0; i < RamBaenke64k; ++i)
    {
        reading_bytes = fread(PsramBaenke[i], 1, 0x10000, file);
...
int REUClass::SaveRAM(const char *filename)
{
    ...
    for(int i = 0; i < RamBaenke64k; ++i) fwrite(PsramBaenke[i], 1, 0x10000, file);
    fclose(file);
    return 0;
}
    ...
    if(TransferStart)
    {
        ...
        else
        {
            ...
            case 0:		// STASH  C64 ---> REU
                PsramBaenke[Bank][AdresseREU] = Read(AdresseC64);
            ...
            case 1:		// FETCH  REU ---> C64
                Write(AdresseC64, PsramBaenke[Bank][AdresseREU]);
                if((IO[10] & 128) != 128) AdresseC64++;
                if((IO[10] & 64) != 64)
                {
                    AdresseREU++;
            ...
            case 2:		// SWAP   C64 <--> REU
                ...
                Write(AdresseC64, PsramBaenke[Bank][AdresseREU]);
                PsramBaenke[Bank][AdresseREU] = tmp1;
                if((IO[10] & 128) != 128) AdresseC64++;
                if((IO[10] & 64) != 64)
                {
                    AdresseREU++;
            ...
            case 3:		// VERIFY C64 ---- REU
                if(PsramBaenke[Bank][AdresseREU] != Read(AdresseC64))
                {
                    TransferStart = false;
                    IO[0] |= 32;
                    IO[0] |= 64;
...
{
    /// REU Inhalt speichern ...
    for(int i=0;i<32;i++) fwrite(PsramBaenke[i],1,0x10000,File);
    return true;
}
