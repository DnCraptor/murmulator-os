const unsigned short PT2_Table[96] = {
    0xef8, 0xe10, 0xd60, 0xc80, 0xbd8, 0xb28, 0xa88, 0x9f0, 0x960, 0x8e0, 0x858, 0x7e0, 0x77c, 0x708, 0x6b0, 0x640, 0x5ec, 0x594,
    0x544, 0x4f8, 0x4b0, 0x470, 0x42c, 0x3fd, 0x3be, 0x384, 0x358, 0x320, 0x2f6, 0x2ca, 0x2a2, 0x27c, 0x258, 0x238, 0x216, 0x1f8,
    0x1df, 0x1c2, 0x1ac, 0x190, 0x17b, 0x165, 0x151, 0x13e, 0x12c, 0x11c, 0x10a, 0xfc, 0xef, 0xe1, 0xd6, 0xc8, 0xbd, 0xb2, 0xa8,
    0x9f, 0x96, 0x8e, 0x85, 0x7e, 0x77, 0x70, 0x6b, 0x64, 0x5e, 0x59, 0x54, 0x4f, 0x4b, 0x47, 0x42, 0x3f, 0x3b, 0x38, 0x35, 0x32,
    0x2f, 0x2c, 0x2a, 0x27, 0x25, 0x23, 0x21, 0x1f, 0x1d, 0x1c, 0x1a, 0x19, 0x17, 0x16, 0x15, 0x13, 0x12, 0x11, 0x10, 0xf
};
...
typedef struct PT1_Channel_Parameters
{
...
typedef struct PT1_Parameters
{
    unsigned char Delay, DelayCounter, CurrentPosition;
} PT1_Parameters;

typedef struct PT1_SongInfo
{
    PT1_Parameters PT1;
...
void PT1_Init(AYSongInfo* info)
{
    ...
    info->data = calloc(1, sizeof(PT1_SongInfo));
    if(!info->data)
        return;
...
void PT1_PatternInterpreter(AYSongInfo* info, PT1_Channel_Parameters* chan)
{
    ...
    do
    {
        ...
        else if(val >= 0x82 && val <= 0x8f)
        {
            ...
            ay_writeay(info, AY_ENV_SHAPE, val - 0x81, 0);
            chan->Address_In_Pattern++;
            ay_writeay(info, AY_ENV_FINE, module[chan->Address_In_Pattern], 0);
            chan->Address_In_Pattern++;
            ay_writeay(info, AY_ENV_COARSE, module[chan->Address_In_Pattern], 0);
        }
        else if(val == 0x90)
            quit = true;
...
void PT1_GetRegisters(AYSongInfo* info, PT1_Channel_Parameters* chan, unsigned char* pTempMixer)
{
    unsigned char *module = info->module;
    unsigned char j, b;
    if(chan->Enabled)
    {
        ...
        if(chan->Envelope_Enabled)
            chan->Amplitude |= 16;
        if((signed char)b < 0)
            *pTempMixer = *pTempMixer | 64;
        else
            ay_writeay(info, AY_NOISE_PERIOD, b & 31, 0);
        if((b & 64) != 0)
            *pTempMixer = *pTempMixer | 8;
...
void PT1_Play(AYSongInfo* info)
{
    ...
    ay_writeay(info, AY_CHNL_A_FINE, PT1_A.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_A_COARSE, (PT1_A.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_B_FINE, PT1_B.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_B_COARSE, (PT1_B.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_C_FINE, PT1_C.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_C_COARSE, (PT1_C.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_A_VOL, PT1_A.Amplitude, 0);
    ay_writeay(info, AY_CHNL_B_VOL, PT1_B.Amplitude, 0);
    ay_writeay(info, AY_CHNL_C_VOL, PT1_C.Amplitude, 0);
}
