...
typedef struct STC_Channel_Parameters
{
...
typedef struct STC_Parameters
{
    unsigned char DelayCounter, Transposition, CurrentPosition;
} STC_Parameters;

typedef struct STC_SongInfo
{
    STC_Parameters STC;
...
void STC_Init(AYSongInfo* info)
{
    ...
    if(info->data)
    {
        free((STC_SongInfo *)info->data);
        info->data = 0;
    }
    info->data = calloc(1, sizeof(STC_SongInfo));
    if(!info->data)
        return;
...
void STC_PatternInterpreter(AYSongInfo* info, STC_Channel_Parameters* chan)
{
    ...
    while(true)
    {
        ...
        {
            ay_writeay(info, AY_ENV_SHAPE, val - 0x80, 0);
            chan->Address_In_Pattern++;
...
void STC_GetRegisters(AYSongInfo* info, STC_Channel_Parameters* chan, unsigned char* pTempMixer)
{
    unsigned short i;
    unsigned char j;
    unsigned char *module = info->module;
    ...
    if(chan->Sample_Tik_Counter >= 0)
    {
        ...
        if((module[i + 1] & 0x80) != 0)
            *pTempMixer = *pTempMixer | 64;
            ...
            ay_writeay(info, AY_NOISE_PERIOD, module[i + 1] & 0x1f, 0);
...
void STC_Play(AYSongInfo* info)
{
    ...
    ay_writeay(info, AY_CHNL_A_FINE, STC_A.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_A_COARSE, (STC_A.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_B_FINE, STC_B.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_B_COARSE, (STC_B.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_C_FINE, STC_C.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_C_COARSE, (STC_C.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_A_VOL, STC_A.Amplitude, 0);
    ay_writeay(info, AY_CHNL_B_VOL, STC_B.Amplitude, 0);
    ay_writeay(info, AY_CHNL_C_VOL, STC_C.Amplitude, 0);
}
