...
void YM_Init(AYSongInfo* info)
{
    ...
    if(info->data)
    {
        free((YM_SongInfo *)info->data);
        info->data = 0;
    }
    info->data = calloc(1, sizeof(YM_SongInfo));
    if(!info->data)
        return;
    YM->Position_In_YM = 0;
    YM->YM_Offset = 0;
...
void YM6i_Play(AYSongInfo* info)
{
    ...
    ay_writeay(info, AY_CHNL_A_FINE, module [k], 0);
    k += info->Length;
    ay_writeay(info, AY_CHNL_A_COARSE, module [k] & 15, 0);
    k += info->Length;
    
    ay_writeay(info, AY_CHNL_B_FINE, module [k], 0);
    k += info->Length;
    ay_writeay(info, AY_CHNL_B_COARSE, module [k] & 15, 0);
    k += info->Length;
    
    ay_writeay(info, AY_CHNL_C_FINE, module [k], 0);
    k += info->Length;
    ay_writeay(info, AY_CHNL_C_COARSE, module [k] & 15, 0);
    k += info->Length;
    
    ay_writeay(info, AY_NOISE_PERIOD, module [k] & 31, 0);
    k += info->Length;
    
    unsigned char mx = module [k] & 63;
    ay_writeay(info, AY_MIXER, mx, 0);
    k += info->Length;
    
    ay_writeay(info, AY_CHNL_A_VOL, module [k] & 31, 0);
    k += info->Length;
    ay_writeay(info, AY_CHNL_B_VOL, module [k] & 31, 0);
    k += info->Length;
    ay_writeay(info, AY_CHNL_C_VOL, module [k] & 31, 0);
    k += info->Length;
    
    ay_writeay(info, AY_ENV_FINE, module [k], 0);
    k += info->Length;
    ay_writeay(info, AY_ENV_COARSE, module [k], 0);
    k += info->Length;
    
    if(module [k] != 255)
        ay_writeay(info, AY_ENV_SHAPE, module [k] & 15, 0);
    YM->Position_In_YM++;
...
{    
    ...
    if(!memcmp(header->Method, "-lh5-", 5))
    {
        ...
        LZHFileHeader *lzh_header = (LZHFileHeader *)module;
        info->module = calloc(LZH_UCompSize, 2);
