...
const unsigned short ASM_Table[] = {
  0xedc, 0xe07, 0xd3e, 0xc80, 0xbcc, 0xb22, 0xa82, 0x9ec, 0x95c, 0x8d6, 0x858, 0x7e0, 0x76e, 0x704, 0x69f, 0x640,
  0x5e6, 0x591, 0x541, 0x4f6, 0x4ae, 0x46b, 0x42c, 0x3f0, 0x3b7, 0x382, 0x34f, 0x320, 0x2f3, 0x2c8, 0x2a1, 0x27b,
  0x257, 0x236, 0x216, 0x1f8, 0x1dc, 0x1c1, 0x1a8, 0x190, 0x179, 0x164, 0x150, 0x13d, 0x12c, 0x11b, 0x10b, 0xfc,
  0xee, 0xe0, 0xd4, 0xc8, 0xbd, 0xb2, 0xa8, 0x9f, 0x96, 0x8d, 0x85, 0x7e, 0x77, 0x70, 0x6a, 0x64, 0x5e, 0x59, 0x54,
  0x50, 0x4b, 0x47, 0x43, 0x3f, 0x3c, 0x38, 0x35, 0x32, 0x2f, 0x2d, 0x2a, 0x28, 0x26, 0x24, 0x22, 0x20, 0x1e, 0x1c
};

typedef struct ASC0_File
{
...
typedef struct ASC_Channel_Parameters
{
...
typedef struct ASC_Parameters
{
    unsigned char Delay, DelayCounter, CurrentPosition;
} ASC_Parameters;

typedef struct ASC_SongInfo
{
    ASC_Parameters ASC;
...
void ASC_Init(AYSongInfo* info)
{
    ...
    info->data = calloc(1, sizeof(ASC_SongInfo));
    if(!info->data)
        return;
...
void ASC_PatternInterpreter(AYSongInfo* info, ASC_Channel_Parameters* chan)
{
    ...
    while(true)
    {
        ...
        {
            ay_writeay(info, AY_ENV_SHAPE, 8, 0);
        }
        else if(val == 0xf9)
        {
            chan->Address_In_Pattern++;
        ...
        {
            ay_writeay(info, AY_ENV_SHAPE, 10, 0);
        }
        else if(val == 0xfb)
        {
            chan->Address_In_Pattern++;
        ...
        {
            ay_writeay(info, AY_ENV_SHAPE, 12, 0);
        }
        else if(val == 0xfe)
        {
            ay_writeay(info, AY_ENV_SHAPE, 14, 0);
        }
        chan->Address_In_Pattern++;
    }
...
void ASC_GetRegisters(AYSongInfo* info, ASC_Channel_Parameters* chan, unsigned char* pTempMixer)
{
    unsigned char *module = info->module;
    signed char j;
    ...
    else
    {
        ...
        *pTempMixer |= (module[chan->Point_In_Sample + 2] & 9) << 3;
        ...
        if(Sample_Says_OK_for_Envelope && ((*pTempMixer & 64) != 0))
            ay_writeay(info, AY_ENV_FINE, ay_readay(info, AY_ENV_FINE, 0) + ((signed char)(module[chan->Point_In_Sample] << 3) / 8), 0);
        else
        ...
        if((*pTempMixer & 64) == 0)
            ay_writeay(info, AY_NOISE_PERIOD, ((unsigned char)(chan->Current_Ton_Sliding >> 8) + chan->Current_Noise) & 0x1f, 0);
...
void ASC_Play(AYSongInfo* info)
{
    ...
    ay_writeay(info, AY_CHNL_A_FINE, ASC_A.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_A_COARSE, (ASC_A.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_B_FINE, ASC_B.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_B_COARSE, (ASC_B.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_C_FINE, ASC_C.Ton & 0xff, 0);
    ay_writeay(info, AY_CHNL_C_COARSE, (ASC_C.Ton >> 8) & 0xf, 0);
    ay_writeay(info, AY_CHNL_A_VOL, ASC_A.Amplitude, 0);
    ay_writeay(info, AY_CHNL_B_VOL, ASC_B.Amplitude, 0);
    ay_writeay(info, AY_CHNL_C_VOL, ASC_C.Amplitude, 0);
}
